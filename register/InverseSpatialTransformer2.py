...
from register.InverseVectorIntegration import InverseVectorIntegration

class InverseSpatialTransformer2(nn.Module):
    def __init__(self, size, int_steps,interp_mode='bilinear', padding_mode='wrap'):
        super(InverseSpatialTransformer2, self).__init__()
        self.size = size
        ...
        self.basic_flows = nn.Parameter(torch.randn(1, 2, *self.size))
        self.vector_integration = InverseVectorIntegration(self.size, int_steps=int_steps)
    ...
    def forward(self, src, flows=None, thetas=None, interp_mode=None, padding_mode=None, align_corners=True, **kwargs):
        if flows is None:
            flows=self.vector_integration(self.basic_flows)
        shape = src.shape[2:]
        assert len(shape) == self.dimension, "Expected volume dimension %s, got %s!" % (self.dimension, len(shape))
